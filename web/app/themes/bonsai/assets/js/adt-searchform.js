...
function copyTile() {
  ...
  clone
    .find(".switch-field-container-contri-analysis")
    .first()
    .children()
    .each(function () {
...
jQuery(document).ready(function ($) {
  ...
  $(
    ...
    let data =
      selectedValue === "person"
        ? await API.get_person_footprint(userSelection)
        : await API.get_product_footprint(userSelection);
  ...
  $("#most-popular ul li button, #search-history-list li").on(
    "click",
    async function (e) {
      e.preventDefault();
  ...
  jQuery("#co2-form-result")
    ...
    .on("change", function () {
      ...
      let amountInput = unitSelect
        .closest(".unit-select-wrapper") // go up to the label wrapping <select>
        .find("input.quantity"); // look inside for input.amount
      let numberInput = amountInput.val();
      let co2_result = unitSelect
        .closest("div.product-result") // go up to the div wrapping
  ...
  $("input.quantity").on("input", function () {
    ...
    let co2_result = amountInput
      .closest("div.product-result") // go up to the div wrapping
      .find("span.co2-value"); // look inside for span.co2-value
    const co2_result_value = localStorage.getItem("result_value");

    let unitSelect = amountInput
      .closest("div.unit-select-wrapper")
      .find("select.unit");
    ...
    let quantityQuestion = amountInput
      .closest("div.contribution-analysis")
  ...
  jQuery(document).on("click", "span.link", async function (e) {
    ...
    userSelection.db_version = jQuery(this)
      .closest(".tile")
...
async function display_result(htmlclass, data) {
  ...
  if (
    data &&
    ((data.error && data.error.includes("Product not found")) ||
      (data.title == "" && data.flow_code == ""))
  ) {
  ...
  main_component
    .find(".product-title")
    .first()
    .attr("data-code", data["flow_code"] ?? "person");
  ...
  main_component
    .find(".emission-unit")
    .first()
    .text("in " + data["unit_emission"]);
  main_component
    .find(".question-location")
    .text(isPersonTab ? data["country"] : Utils.capitalize(data["title"]));
  ...
  main_component
    .find(".co2-value")
    .first()
    .text(Utils.reformatValue(default_selected_unit_ratio * data["value"]));
  ...
  let selectedUnit_dropdownlist = main_component
    .find("select.unit")
  ...
  main_component
    .find(".co2-value-unit")
    .text(isPersonTab ? CONST.UNIT.TONNESCO2 : co2Value_unit);
...
function display_recipe_table(
  ...
  for (const recipe of recipeArray) {
    ...
    if (
      (recipe.value_emission && recipe.value_emission !== NaN) ||
    ...
    const selectedUnit_dropdownlist = main_component
      .find("select.unit")
      .find("option:selected")
      .text();
    let displayed_unit = Utils.getUnitContriAnalysis(
    ...
    if (
      displayed_unit &&
      displayed_unit["label"] !== null &&
      displayed_unit["label"] !== undefined &&
      displayed_unit["label"] !== "" &&
      displayed_unit["label"].includes("tonnes")
    ) {
    ...
    let value_inflow = recipe.value_inflow
      ? Utils.reformatValue(recipe.value_inflow * displayed_unit["ratio"])
      : "";

    if (
      recipe.inflow != null &&
      (recipe.inflow.toLowerCase() === "other" ||
        recipe.inflow.toLowerCase() === "direct")
    ) {
    ...
    if (
      recipe.flow_input != null &&
      (recipe.flow_input.toLowerCase() === "other" ||
        recipe.flow_input.toLowerCase() === "direct")
    ) {
...
function adt_dynamic_search_input(list_product) {
  ...
  $input.on("input", function () {
    ...
    if (matches.length > 0 && query) {
      jQuery(this)
        .css("border-radius", "50px 50px 0 0")
...
function adt_save_local_search_history(userSelection) {
  ...
  searchHistory.forEach((search, index) => {
    searchHistoryHtml +=
      '<li class="button primary is-outline" style="border-radius:99px;" data-code="' +
      search.productCode +
      '" data-uuid="' +
      search.productUuid +
      '">';
